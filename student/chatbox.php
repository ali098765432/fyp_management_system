...
$getuseridvalue= $fetchdata['id'];
 $getstdegree= $fetchdata['degree'];
  $getstdfullname= $fetchdata['firstname']. "" .
  $fetchdata['lastname'];
   $valofid = $fetchdata['id'];

//fetching supervisor

   $xsupid="SELECT * FROM proposalfiles WHERE 
uid='$getuseridvalue' AND status='Accepted'";

$xrunspid=mysqli_query($conn,$xsupid);
$xx=mysqli_fetch_array($xrunspid);
 ...
 WHERE tou='$valofid' AND fromu='$xspidval'";
 $updatemsgrun=mysqli_query($conn, $updatemsgstat);




if(mysqli_num_rows($xrunspid)>=1){

 //fetching supervisor dtails
$seleupdetail="SELECT * FROM supervisor WHERE
    id='$xspidval'";
    $runningseledet=mysqli_query($conn,$seleupdetail);
    $runningsupdeta=mysqli_fetch_array($runningseledet);

 $detailsupname=$runningsupdeta['name'];
 $uname =$runningsupdeta['name'];
...
<div class="card shadow-lg border-1 rounded-lg mt-5">
 ...
 <div class="card-header">
                <h4 class="card-title"><strong>Supervisor:<b> <?php echo $uname;?></b></strong></h4>
...
?>
    ...
    <input type="hidden" id="fromuname" value="<?php echo $getstdfullname  ?>"  name="fromuname">

    <input type="hidden" id="tou" value="<?php echo  $xspidval ?>" name="tou">

     <input type="hidden" id="touname" value="<?php echo $uname ?>" name="touname"><br>
...
<?php
     ...
     echo "<script> alert('No Supervisor Selected Your Proposal Yet, Once Supervisor Accepted Your Proposal then you will be able to chat with your supervisor')</script>";

      echo "<script> window.location=
 'userdashboard.php'</script>";
 }
