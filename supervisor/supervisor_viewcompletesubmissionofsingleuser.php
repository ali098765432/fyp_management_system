...
$fetchingselviewmarks=mysqli_fetch_array($runingselviewmarks);
  ...
  <div class="form-group"  style="margin-top:10px;margin-right:10px;margin-left:10px;">
    ...
    <input type="number" class="form-control" placeholder="Enter Proposal Marks" name="stdpromarks" value="<?php echo $fetchingselviewmarks['proposalmarks']?>" min='1'  max='10'>
  </div>

  <div class="form-group"  style="margin-top:10px;margin-right:10px;margin-left:10px;">
    <label style="text-align:left;">Mid Marks</label>
    <input type="number" class="form-control" placeholder="Enter Mid Marks" name="stdmidmarks" value="<?php echo $fetchingselviewmarks['midmarks']?>" min='1' max='30'>
  </div>


  <div class="form-group"  style="margin-top:10px;margin-right:10px;margin-left:10px;">
    <label style="text-align:left;">Report Marks</label>
    <input type="number" class="form-control" placeholder="Enter Report Marks" name="stdreportsubmarks" value="<?php echo $fetchingselviewmarks['reportmarks']?>" min='1' max='10'>
  </div>

  <div class="form-group"  style="margin-top:10px;margin-right:10px;margin-left:10px;">
    <label style="text-align:left;">Final Submission Marks</label>
    <input type="number" class="form-control" placeholder="Enter Final Submission Marks" name="stdfinalsubmarks" value="<?php echo $fetchingselviewmarks['finalsubmarks']?>" min='1' max='50'>
  </div>
...
$gettingupid=$_GET['pid'];

$selpropdetailsb="select * from projectsubmissions where pid ='$gettingupid'";
...
?>
        ...
        <tr>
            ...
            <td><a href="../student/submissionfiles/<?php echo $rowpropsb['submitfile']; ?>" class="btn btn-success">Download File</a>
                ...
                <a href="addsubmissioncomment.php?psbid=<?php echo  $rowpropsb['id'];?>&uid=<?php  echo $getsupid?>" class="btn btn-primary">Add Comments</a>
...
if($runingstdalreadyup){
     ...
     echo "<script> alert('Marks Updated Successfully ')</script>";
      $innoti = "INSERT INTO notification(msg,userid,sentnotitime,status,type) VALUES ('Supervisor Updated Your aassignment marks','$getstdidup',Now(),'unread','marks')";

       $runnotiin = mysqli_query($conn,$innoti);

      //  echo "<script> window.location=
      // 'supervisor_viewcompletesubmissionofsingleus//er.php?pid=$getuseridvalue'</script>";
...
if($numofrecordalready>0){
   ...
   $insertcol="INSERT INTO studentmarks(proposalmarks,midmarks, reportmarks,finalsubmarks,studentid) VALUES ('$getproposalmarks','$getmidmarks',
      ...
      if($runcol){
           ...
           echo "<script> alert('Marks Updated Successfully ')</script>";
      $innoti = "INSERT INTO notification(msg,userid,sentnotitime,status,type) VALUES ('Supervisor added Your aassignment marks','$getstdid',Now(),'unread','marks')";
